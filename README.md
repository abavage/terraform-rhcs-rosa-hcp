...
| <a name="input_auto_node_mode"></a> [auto\_node\_mode](#input\_auto\_node\_mode) | AutoNode mode. Currently only enabled is supported. | `string` | `null` | no |
| <a name="input_auto_node_role_arn"></a> [auto\_node\_role\_arn](#input\_auto\_node\_role\_arn) | AWS IAM role ARN used by the Karpenter controller when AutoNode is enabled. | `string` | `null` | no |
